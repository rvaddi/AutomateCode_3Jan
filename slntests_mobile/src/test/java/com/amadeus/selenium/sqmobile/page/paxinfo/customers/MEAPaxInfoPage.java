...
/**
 * MEAAirline PaxInfopage
 * @author vbalasubramanian
 *
 */
public class MEAPaxInfoPage extends PaxInfoPage{
  ...
  public MEAPaxInfoPage(SeleniumSEPTest test) throws Exception {
    super(test);
    // Checking if we are on the right page
    if(! WaitUtils.waitForElementPresent(getTest(), LOC_IN_PAX_INFO_PAX_DETAILS_FIRST_NAME, 30)){
      reporter.fail("This is not MEAPaxInfo Page");
    }else{
      reporter.reportPassed("MEAPaxInfo Page", "In MEAPaxInfo Page");
    }
  }
